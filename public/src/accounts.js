...
function sortAccountsByLastName(accounts) {
  ...
  const result = accounts.sort((a,b) => a.name.last.toLowerCase() > b.name.last.toLowerCase() ? 1 : -1);
  return result;
}
...
function getAccountFullNames(accounts) {
  ...
  const result = accounts.map((element) => `${element.name.first} ${element.name.last}`);
  return result;
}
